...
async def on_message(message):
    # making sure the bot doesnt reply to itself
    ...
    if message.content.startswith('!tip '):
        ...
        if (not (to_tip_mention[:2] == '<@')):
            return await message.channel.send('{0.author.mention}, invalid account.'.format(message))
        try:
            amount = float(amount)
        except ValueError:
            return await message.channel.send('{0.author.mention}, invalid amount.'.format(message))
        try:
            c.tip(tipper, to_tip, amount)
            price_usd = m.get_price(float(amount))[1]
            return await message.channel.send('{0.author.mention} has tipped %s %f %s ($%.2f).'.format(message) % (to_tip_mention, amount, cs['ticker'], price_usd))
        except ValueError:
            return await message.channel.send('{0.author.mention}, insufficient balance.'.format(message))
    ...
    if message.content.startswith('!withdraw '):
        ...
        if (not c.validate_address(address)):
            return await message.channel.send('{0.author.mention}, invalid address.'.format(message))
        try:
            amount = float(amount)
        except ValueError:
            return await message.channel.send('{0.author.mention}, invalid amount.'.format(message))
        try:
            ...
            return await message.channel.send('{0.author.mention}, withdrawal complete, TXID %s'.format(message) % txid)
        except ValueError:
            return await message.channel.send('{0.author.mention}, insufficient balance.'.format(message))
    ...
    if message.content.startswith('!rain '):
        ...
        if amount < 0.01:
            return await message.channel.send('{0.author.mention}, the amount must be bigger than 0.01 %s.'.format(message) % cs['ticker'])
        try:
            amount = float(amount)
        except ValueError:
            return await message.channel.send('{0.author.mention}, invalid amount.'.format(message))
        try:
            ...
            return await message.channel.send('{0.author.mention} has tipped %f %s to everyone on this server!'.format(message) % (eachtip, cs['ticker']))
        except ValueError:
            return await message.channel.send('{0.author.mention}, insufficient balance.'.format(message))
